...
import { Settings, User, Shield, LogOut, Trash2, Save, Mail, ChevronLeft, ChevronRight } from 'lucide-react';
...
const SettingsPage = () => {
  ...
  const panelLabels = isAdmin ? ['Profile', 'Account', 'Admin'] : ['Profile', 'Account'];
  ...
  const onPointerUp = useCallback(() => {
    ...
    setDragOffset(0);
  }, [isDragging, dragOffset, panelIndex, panelCount]);

  const baseTranslate = -(panelIndex * 100);
  const dragPct = trackRef.current ? (dragOffset / trackRef.current.offsetWidth) * 100 : 0;
  const clampedTranslateX = Math.max(-(panelCount - 1) * 100, Math.min(0, baseTranslate + dragPct));
